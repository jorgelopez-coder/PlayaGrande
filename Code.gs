...
// === Lectura por lote (2026-09-15) =======================================
// planilla.html (y potencialmente otras pantallas RRHH) dispara entre 8 y 10
// llamadas ?modulo=X EN PARALELO al cargar — eso choca seguido contra el
// límite de ejecuciones simultáneas de Apps Script por usuario, causando
// fallas intermitentes (503/404/página de error de Drive) que no dejan
// rastro en "Ejecuciones" porque Google las rechaza ANTES de invocar
// doGet(). ?modulo=lote junta varios módulos "simples" (los que solo
// devuelven filas de una hoja, sin lógica de preview) en UNA sola llamada:
// e.parameter.modulos = "personal,vacaciones,feriados,..." (separados por
// coma) → { ok:true, resultados: { personal: {...}, vacaciones: {...}, ... } }.
// Reutiliza rrhhConCache por módulo individual, así que se beneficia del
// mismo cache de 120s que ya tenían las llamadas sueltas.
var MODULOS_HOJA_SIMPLE = {
  personal:          [HOJA_PERSONAL, ENCABEZADOS_PERSONAL],
  vacaciones:        [HOJA_VACACIONES, ENCABEZADOS_VACACIONES],
  permisos_sin_goce: [HOJA_PERMISOS_SIN_GOCE, ENCABEZADOS_PERMISOS_SIN_GOCE],
  amonestaciones:    [HOJA_AMONESTACIONES, ENCABEZADOS_AMONESTACIONES],
  terminaciones:     [HOJA_TERMINACIONES, ENCABEZADOS_TERMINACIONES],
  cambios_salario:   [HOJA_CAMBIOS_SALARIO, ENCABEZADOS_CAMBIOS_SALARIO],
  movimientos:       [HOJA_MOVIMIENTOS, ENCABEZADOS_MOVIMIENTOS],
  liquidaciones:     [HOJA_LIQUIDACIONES, ENCABEZADOS_LIQUIDACIONES],
  aguinaldos:        [HOJA_AGUINALDOS, ENCABEZADOS_AGUINALDOS],
  horarios:          [HOJA_HORARIOS, ENCABEZADOS_HORARIOS],
  horarios_estado:   [HOJA_HORARIOS_ESTADO, ENCABEZADOS_HORARIOS_ESTADO],
  feriados:          [HOJA_FERIADOS, ENCABEZADOS_FERIADOS],
  incidencias:       [HOJA_INCIDENCIAS, ENCABEZADOS_INCIDENCIAS],
  horas_extra:       [HOJA_HORAS_EXTRA, ENCABEZADOS_HORAS_EXTRA],
  planillas:         [HOJA_PLANILLAS, ENCABEZADOS_PLANILLAS],
  planillas_detalle: [HOJA_PLANILLAS_DETALLE, ENCABEZADOS_PLANILLAS_DETALLE],
  servicio_repartos: [HOJA_SERVICIO_REPARTOS, ENCABEZADOS_SERVICIO_REPARTOS],
  servicio_detalle:  [HOJA_SERVICIO_DETALLE, ENCABEZADOS_SERVICIO_DETALLE],
  roles:             [HOJA_ROLES, ENCABEZADOS_ROLES],
  usuarios:          [HOJA_USUARIOS, ENCABEZADOS_USUARIOS]
};

// Mismo resultado que devuelve el case correspondiente del switch de doGet
// para un módulo "simple" (hoja completa, sin lógica de preview) — usado
// tanto por ?modulo=lote como, potencialmente, por futuras llamadas sueltas.
// 'kioskos' no está acá porque su respuesta trae un campo extra (ver
// doGet) — se resuelve aparte dentro del case 'lote'.
function obtenerModuloSimple(modulo) {
  const par = MODULOS_HOJA_SIMPLE[modulo];
  if (!par) return { ok: false, error: 'Módulo no reconocido: ' + modulo };
  return rrhhConCache(modulo, function () {
    return { ok: true, registros: filasComoObjetos(prepararHoja(par[0], par[1])) };
  });
}
  ...
  try {
    ...
    switch (modulo) {
      ...
      case 'usuarios':
        ...
        break;
      case 'lote': {
        // Ver nota arriba de MODULOS_HOJA_SIMPLE/obtenerModuloSimple.
        const modulosPedidos = String(e.parameter.modulos || '')
          .split(',').map(function (m) { return m.trim(); }).filter(Boolean);
        if (!modulosPedidos.length) return jsonOut({ ok: false, error: 'Falta el parámetro modulos.' });
        const resultados = {};
        modulosPedidos.forEach(function (m) {
          if (m === 'kioskos') {
            resultados[m] = rrhhConCache('kioskos', function () {
              const h = prepararHoja(HOJA_CONFIGURACION, ENCABEZADOS_CONFIGURACION);
              return { ok: true, registros: filasComoObjetos(h), kioskos: obtenerKioskosActivos() };
            });
          } else {
            resultados[m] = obtenerModuloSimple(m);
          }
        });
        return jsonOut({ ok: true, resultados: resultados });
      }
      default:
